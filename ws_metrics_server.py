...
# Cloud PaaS platforms (Render, Railway, Fly.io) inject PORT and expect the
# process to bind 0.0.0.0:$PORT. Falls back to localhost:8765 for local dev.
...
async def main():
    async with websockets.serve(stream_metrics, HOST, PORT):
        print(f"[ws] metrics stream ready at ws://{HOST}:{PORT}")
